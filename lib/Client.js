...
class Client extends EventEmitter {
	...
	* @arg {String} guildID The ID of the guild
	* @arg {Object} options The search options. See [the official Discord API documentation](https://discordapp.com/developers/docs/resources/) for object structure
	* @returns {Promise<Array>} An array of search results (arrays). The messages have the added property hit
	*/
	searchGuildMessages(guildID, options) {
		return this.requestHandler.request("GET", Endpoints.SEARCH_GUILD(guildID), true, options).then((messages) => messages.map((group) => {
			return group.map((message) => new Message(message, this).hit = !!message.hit);
		}));
	}
	...
	* @returns {Promise<Array>} An array of search results (arrays). The messages have the added property hit
	*/
	searchChannelMessages(channelID, options) {
		return this.requestHandler.request("GET", Endpoints.SEARCH_CHANNEL(channelID), true, options).then((messages) => messages.map((group) => {
			return group.map((message) => new Message(message, this).hit = !!message.hit);
		}));
	}
